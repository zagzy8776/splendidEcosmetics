...
export default function App() {
  ...
  function saveCategoryImages(imgs: CategoryImages) {
  ...
  return (
    <div style={{ fontFamily: "'Raleway', sans-serif", backgroundColor: "#FFF6F3", minHeight: "100dvh", overflowX: "hidden" }}>
      ...
      <CategorySection active={activeCategory} onSelect={setActiveCategory} categories={allCategories} categoryImages={categoryImages} />
...
function CategorySection({ active, onSelect, categories, categoryImages }: { active: Category; onSelect: (c: Category) => void; categories: Category[]; categoryImages: CategoryImages }) {
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px" }}>
        ...
        >
          {cats.map(cat => {
            ...
            const img = categoryImages[cat] ?? (cat === "All" ? "https://images.unsplash.com/photo-1522335789203-aabd1fc54bc9?w=150&h=150&fit=crop" : (CAT_IMAGES[cat as Exclude<Category, "All">] ?? "https://images.unsplash.com/photo-1522335789203-aabd1fc54bc9?w=150&h=150&fit=crop"));
            return (
              <button 
                key={cat} 
                onClick={() => {
          ...
          <>
            ...
            {totalPages > 1 && (
              <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 6, marginTop: 40 }}>
                {/* PREV */}
                <button
                  onClick={() => { setCurrentPage(p => p - 1); document.getElementById("products")?.scrollIntoView({ behavior: "smooth" }); }}
                  disabled={currentPage === 1}
                  style={{ padding: "8px 14px", borderRadius: 999, fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", border: "1px solid rgba(201,162,39,0.3)", background: "transparent", color: currentPage === 1 ? "#d1d5db" : "#5C3D2E", cursor: currentPage === 1 ? "not-allowed" : "pointer", whiteSpace: "nowrap" }}
                >← PREV</button>

                {/* Smart page numbers: always show at most 5 buttons + ellipsis */}
                {(() => {
                  const pages: (number | "...")[] = [];
                  if (totalPages <= 5) {
                  ...
                  return pages.map((p, idx) =>
                    p === "..." ? (
                      <span key={`ellipsis-${idx}`} style={{ color: "#9A7A6E", fontSize: 13, padding: "0 2px" }}>…</span>
                    ) : (
                      <button
                        key={p}
                        onClick={() => { setCurrentPage(p as number); document.getElementById("products")?.scrollIntoView({ behavior: "smooth" }); }}
                        style={{ width: 36, height: 36, borderRadius: "50%", fontSize: 12, fontWeight: 700, border: p === currentPage ? "none" : "1px solid rgba(201,162,39,0.25)", background: p === currentPage ? "linear-gradient(135deg, #C9A227 0%, #A8841A 100%)" : "transparent", color: p === currentPage ? "#fff" : "#5C3D2E", cursor: "pointer", boxShadow: p === currentPage ? "0 4px 12px rgba(201,162,39,0.25)" : "none", transition: "all 0.2s", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}
                      >{p}</button>
                    )
                  );
                })()}
                ...
                <button
                  ...
                  style={{ padding: "8px 14px", borderRadius: 999, fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", border: "1px solid rgba(201,162,39,0.3)", background: "transparent", color: currentPage === totalPages ? "#d1d5db" : "#5C3D2E", cursor: currentPage === totalPages ? "not-allowed" : "pointer", whiteSpace: "nowrap" }}
                >NEXT →</button>
              </div>
            )}
          </>
    ...
    <section id="location" style={{ background: "linear-gradient(180deg, #FFFDFC 0%, #FFF6F3 100%)" }} className="py-10 sm:py-14">
      <div style={{ maxWidth: 1200, margin: "0 auto" }} className="px-4 sm:px-6">
        <div style={{ borderRadius: 24, overflow: "hidden", display: "grid", boxShadow: "0 24px 70px rgba(26,15,10,0.16)", border: "1px solid rgba(249,222,218,0.28)" }} className="grid-cols-1 md:grid-cols-2">
          ...
          <div style={{ position: "relative", minHeight: "350px", background: "radial-gradient(circle at 60% 40%, rgba(201,162,39,0.18) 0%, transparent 55%), linear-gradient(145deg, #1A0F0A 0%, #2A160E 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 0 }} className="min-h-[280px] sm:min-h-[350px] md:min-h-full overflow-hidden">
            {/* Decorative rings */}
            <div style={{ position: "absolute", width: 320, height: 320, borderRadius: "50%", border: "1px solid rgba(201,162,39,0.12)", top: "50%", left: "50%", transform: "translate(-50%, -50%)", pointerEvents: "none" }} />
            <div style={{ position: "absolute", width: 240, height: 240, borderRadius: "50%", border: "1px solid rgba(201,162,39,0.18)", top: "50%", left: "50%", transform: "translate(-50%, -50%)", pointerEvents: "none" }} />
            <div style={{ position: "absolute", width: 160, height: 160, borderRadius: "50%", border: "1px solid rgba(201,162,39,0.25)", top: "50%", left: "50%", transform: "translate(-50%, -50%)", pointerEvents: "none" }} />
            {/* Logo */}
            <div style={{ position: "relative", zIndex: 2, display: "flex", flexDirection: "column", alignItems: "center", gap: 16, padding: "40px 24px" }}>
              <div style={{ width: 110, height: 110, borderRadius: "50%", padding: 4, background: "linear-gradient(135deg, #C9A227 0%, #A8841A 100%)", boxShadow: "0 0 0 6px rgba(201,162,39,0.15), 0 20px 50px rgba(0,0,0,0.4)" }}>
                <img src="/logo.jpg" alt="Splendid Empire Cosmetics" style={{ width: "100%", height: "100%", borderRadius: "50%", objectFit: "cover", display: "block" }} />
              </div>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, fontWeight: 700, color: "#fff", letterSpacing: "0.04em", lineHeight: 1.2 }}>Splendid Empire</div>
                <div style={{ fontSize: 9, letterSpacing: "0.45em", color: "#C9A227", fontWeight: 700, marginTop: 4 }}>COSMETICS</div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                {Array.from({ length: 5 }).map((_, i) => <Star key={i} size={11} fill="#C9A227" color="#C9A227" />)}
                <span style={{ color: "#9A7A6E", fontSize: 11, marginLeft: 6 }}>Owerri's #1 Beauty Store</span>
              </div>
              <div style={{ background: "rgba(201,162,39,0.12)", border: "1px solid rgba(201,162,39,0.25)", borderRadius: 999, padding: "6px 16px", fontSize: 10, color: "#C9A227", fontWeight: 700, letterSpacing: "0.15em" }}>AUTHENTIC · IMPORTED · TRUSTED</div>
            </div>
          </div>
        </div>
      </div>
...
function AdminPanel({ products, setProducts, orders, setOrders, onExit, categoryImages, onSaveCategoryImages }: { products: Product[]; setProducts: React.Dispatch<React.SetStateAction<Product[]>>; orders: Order[]; setOrders: React.Dispatch<React.SetStateAction<Order[]>>; onExit: () => void; categoryImages: CategoryImages; onSaveCategoryImages: (imgs: CategoryImages) => void }) {
  ...
  return (
    <div style={{ height: "100dvh", backgroundColor: "#FFF6F3", fontFamily: "'Raleway', sans-serif", display: "flex", flexDirection: "column", overflow: "hidden", width: "100%", boxSizing: "border-box" }}>
      ...
      <div ref={contentRef} style={{ flex: 1, overflowY: "auto", overflowX: "hidden", WebkitOverflowScrolling: "touch" } as React.CSSProperties}>
        ...
        <div style={{ maxWidth: 1060, margin: "0 auto", padding: "28px 16px 20px", boxSizing: "border-box" }}>
          ...
          {tab === "categories" && <AdminCategories products={products} setProducts={setProducts} categoryImages={categoryImages} onSaveCategoryImages={onSaveCategoryImages} />}
...
function AdminCategories({ products, setProducts, categoryImages, onSaveCategoryImages }: { products: Product[]; setProducts: React.Dispatch<React.SetStateAction<Product[]>>; categoryImages: CategoryImages; onSaveCategoryImages: (imgs: CategoryImages) => void }) {
  ...
  async function handleCatImageUpload(e: React.ChangeEvent<HTMLInputElement>, cat: string) {
    const file = e.target.files?.[0];
    if (!file) return;
    e.target.value = "";
    const allowed = ["image/jpeg", "image/jpg", "image/png", "image/webp", "image/heic", "image/heif"];
    if (!allowed.includes(file.type)) { flash("err", "Please upload a JPEG, PNG, or WebP image."); return; }
    if (file.size > 15 * 1024 * 1024) { flash("err", "Image too large. Max 15MB."); return; }
    setUploadingCat(cat);
    try {
      const url = await cloudinaryUpload(file);
      await saveCategoryPhoto(cat, url);
      onSaveCategoryImages({ ...categoryImages, [cat]: url });
      flash("ok", `Image updated for "${cat}"`);
    } catch (err: any) {
  ...
  async function removeCatImage(cat: string) {
    try {
      await saveCategoryPhoto(cat, null);
      const next = { ...categoryImages };
      delete next[cat];
      onSaveCategoryImages(next);
      flash("ok", `Photo removed for "${cat}"`);
    } catch (err: any) {
      flash("err", "Failed to remove photo.");
    }
  }
    ...
    Promise.all(toUpdate.map(p => updateProduct(p.id, { category: trimmed })))
      .then(async () => {
        ...
        if (categoryImages[cat]) {
          ...
          await saveCategoryPhoto(trimmed, catImg);
          ...
          categories.map(cat => {
            ...
            return (
              <div key={cat} style={{ background: "#fff", borderRadius: 14, padding: "14px 18px", border: "1px solid rgba(249,222,218,0.2)", display: "flex", alignItems: "center", gap: 12, boxShadow: "0 1px 6px rgba(201,162,39,0.06)" }}>
                {/* Category image upload slot */}
                <div style={{ position: "relative", flexShrink: 0 }}>
                  <input
                    id={`cat-img-${cat}`}
                    type="file"
                    accept="image/jpeg,image/jpg,image/png,image/webp,image/heic"
                    onChange={e => handleCatImageUpload(e, cat)}
                    disabled={uploadingCat !== null}
                    style={{ display: "none" }}
                  />
                  <label
                    htmlFor={`cat-img-${cat}`}
                    title="Click to change category photo"
                    style={{
                      display: "block", width: 52, height: 52, borderRadius: "50%", overflow: "hidden",
                      border: `2px dashed ${categoryImages[cat] ? "#C9A227" : "rgba(201,162,39,0.4)"}`,
                      cursor: uploadingCat !== null ? "not-allowed" : "pointer",
                      background: "#FFF6F3", flexShrink: 0, position: "relative",
                    }}
                  >
                    {uploadingCat === cat ? (
                      <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <div style={{ width: 18, height: 18, border: "2px solid rgba(201,162,39,0.2)", borderTopColor: "#C9A227", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
                      </div>
                    ) : categoryImages[cat] ? (
                  ...
                  {categoryImages[cat] && uploadingCat !== cat && (
                    <button
                      onClick={() => removeCatImage(cat)}
                      title="Remove photo"
                      style={{ position: "absolute", top: -4, right: -4, width: 18, height: 18, borderRadius: "50%", background: "#ef4444", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 10, fontWeight: 700, lineHeight: 1 }}
                    >×</button>
                  )}
                </div>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {!isRenaming && <p style={{ color: "#9A7A6E", fontSize: 10, margin: "3px 0 0", letterSpacing: "0.04em" }}>Tap 📷 to set category photo</p>}
                </div>
